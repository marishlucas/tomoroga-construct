...
- Native page scrolling drives a reversible GSAP sequence. Mobile hides the chapter-button bar, and the decorative canvas does not receive touch input. Chapter controls jump to stable reading positions. Mobile uses a shorter sequence with readable copy above the scene. The user explicitly requests continuous 3D scroll animation even when the OS prefers reduced motion; decorative entrances still honor that preference; retain a useful WebGL fallback.
