...
- Native page scrolling drives a reversible GSAP sequence. Mobile hides the chapter-button bar, and the decorative canvas does not receive touch input. Chapter controls jump to stable reading positions. Mobile uses a shorter sequence with readable copy above the scene. Respect reduced motion with discrete chapter changes driven by scrolling; retain a useful WebGL fallback.
